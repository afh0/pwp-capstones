...
def get_average_sentence_length(text):
    ...
    sentence_lengths = []
    for item in sentences_in_text:
        sentence_lengths.append(len(item.strip(' ').split(' ')))
        sentence_length_total = 0
        for item in sentence_lengths:
            sentence_length_total += item
    return (sentence_length_total - 1)/len(sentence_lengths)
...
def prepare_text(text):
    for words in text:
        prepared_text = text.lower().replace('?', ' ').replace('!', ' ').replace(',', ' ').replace('.', ' ').split()
...
def ngram_creator(text_list):
    ...
    for element in range(len(text_list)-1):
        ngram_list.append(' '.join(text_list[element:element+2]))
    return ngram_list
...
class TextSample:
    def __init__(self, text, author):
        ...
        self.word_count_frequence = build_frequency_table(self.prepared_text)
        self.ngram_frequency = build_frequency_table(ngram_creator(self.prepared_text))

        

    def __repr__(self):
        return "{author} has an average sentence length of {length} words.".format(author=self.author, length=self.average_sentence_length)
...
def find_text_similarity(sample1, sample2):
    ...
    word_count_similarity = frequency_comparison(sample1.word_count_frequence, sample2.word_count_frequence)
...
print("Lily Trebuchet has a similiarity score of " + str(find_text_similarity(murderer_sample, lily_sample)) + ".")
print("Myrtle Beech has a similarity score of " + str(find_text_similarity(murderer_sample, myrtle_sample)) + ".")
print("Gregg T. Fishy has a similarity score of " +str(find_text_similarity(murderer_sample, gregg_sample)) + ".")
